...
export async function addSourceUrl(notebookId, url) {
  // Format correct selon rpc-reference.md (_sources.py::_add_url_source)
  // L'URL va à la position [2] dans un tableau de 8 éléments
  const params = [
    [[null, null, [url], null, null, null, null, null]],  // [0]: source config, URL à [0][0][2]
    notebookId,   // [1]: notebook ID
    [2],          // [2]: flag type source
    null,
    null,
  ];

  const data = await rpcCall(RPC.ADD_SOURCE, params, `/notebook/${notebookId}`);
  return data;
}
